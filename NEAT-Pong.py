...
def evaluate_genomes(genomes, config):
    ...
    # Start new game instance for every network == genome
...
def train_ai(game, genome1, config):
    ...
    while run:
        ...
        if time.time() - start >= 0.5:
            calculate_fitness(genome1, game)
            break
...
def get_network_inputs(paddle, ball):
    return [paddle.x, ball.x]


def calculate_fitness(genome, game):
    genome.fitness = game.left_score
...
if __name__ == "__main__":
    ...
    if train:
        ...
        population = neat.Checkpointer.restore_checkpoint('Checkpoints/checkpoint-56')
